...
export default async function FinancialReportPage({ searchParams }: { searchParams: Promise<ReportSearchParams> }) {
  const { user } = await requireReportsPage("financial");
  ...
  const {
    ...
    invoiceStatus,
    vendorBillStatus,
  } = await getFinancialReportRows(params);
  ...
  const bucketLabel = agingBucketLabel;
  ...
  const csvHref = (report: string) =>
    `/api/exports/reports/financial?report=${report}&from=${range.fromInput}&to=${range.toInput}`;
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {hasPermission(user, "exports:csv") ? (
          <div className="flex flex-wrap gap-2">
            <Button asChild size="sm" variant="outline"><a download href={csvHref("profit")}>Download Profit CSV</a></Button>
            <Button asChild size="sm" variant="outline"><a download href={csvHref("closeout")}>Download Closeout CSV</a></Button>
            <Button asChild size="sm" variant="outline"><a download href={csvHref("receivables")}>Download Receivables CSV</a></Button>
            <Button asChild size="sm" variant="outline"><a download href={csvHref("payables")}>Download Payables CSV</a></Button>
          </div>
        ) : null}
      </div>
